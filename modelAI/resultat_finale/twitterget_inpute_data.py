...
def get_details(username,access_key="",access_secret=""):
    ...
    return {"user_data":(user_data),"chart":monday_to_sunday}


def get_followers_details(username,access_key="",access_secret=""):
    followers=[]
    try:
        consumer_key = config('TWITTER_CONSUMER_KEY')
        consumer_secret = config('TWITTER_CONSUMER_SECRET')
        ...
        api = tweepy.API(auth, wait_on_rate_limit=True)
        userdata = api.followers_ids(screen_name=username, count=200)
    ...
    for user_id in userdata:    
        ...
        try: 
            ...
            userdata = api.get_user(user_id=user_id)
            user = userdata._json
            timeline = api.user_timeline(
            user_id=user_id, count=200, include_rts=True, tweet_mode="extended")
...
def get_friends_details(username,access_key="",access_secret=""):
    friends=[]
    try:
        consumer_key = config('TWITTER_CONSUMER_KEY')
        consumer_secret = config('TWITTER_CONSUMER_SECRET')
        ...
        api = tweepy.API(auth, wait_on_rate_limit=True)
        userdata = api.friends_ids(screen_name=username, count=200)

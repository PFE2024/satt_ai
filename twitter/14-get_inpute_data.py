...
def get_details(username,access_key,access_secret):
    data = pd.read_csv("./Data/data+following_years+followers_lastpost.csv")
    following_years_avg =round(data[data['account_type'] == 1].groupby('date_joined')['following'].mean(),0) 
    following_years_avg = following_years_avg.rename('following_avg')
    followers_years_avg =round(data[data['account_type'] == 1].groupby('most_recent_post')['followers'].mean(),0)
    ...
    lang_dict = pd.read_csv("./Data/lang_dict1.csv", index_col=0).squeeze("columns")
    ...
    lang_dict.to_csv('./Data/lang_dict1.csv')
    ...
    following_years=following_years_avg[dateofjoin]- user["friends_count"]
    followers_lastpost=followers_years_avg[most_recent_post]-user["followers_count"]
    user_data = {
...
"favourites": user["favourites_count"],
 ...
 "url": user["url"],
        ...
        "following/years":following_years,
        "followers/lastpost":followers_lastpost,
